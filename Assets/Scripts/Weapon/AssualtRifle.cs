...
namespace Scripts.Weapon
{
    public class AssualtRifle : Firearms
    {
        ...
        private void CreateBullet()
        {
            ...
            //GameObject tmp_Bullet = PoolManager.Instance.GetObject(BulletPrefab, MuzzlePoint.position, MuzzlePoint.rotation)
            GameObject tmp_Bullet = GameObjectPool.Instance.GetGameObject(
            BulletPrefab.name, MuzzlePoint.position, MuzzlePoint.rotation);
            tmp_Bullet.transform.eulerAngles += CalculateSpreadOffset();
            tmp_Bullet.transform.SetParent(MuzzlePoint);
            StartCoroutine(DelayEnterPool(tmp_Bullet));
        }

        private IEnumerator DelayEnterPool(GameObject obj)
        {
